...
public class OldNativeSocketAddress {
     ...
     */
    static OldNativeSocketAddress[] allocate(int count) {
        OldNativeSocketAddress[] array = new OldNativeSocketAddress[count];
        for (int i = 0; i < count; i++) {
            try {
                array[i] = new OldNativeSocketAddress();
     ...
     */
    static void freeAll(OldNativeSocketAddress[] array) {
        for (int i = 0; i < array.length; i++) {
            OldNativeSocketAddress sa = array[i];
            if (sa != null) {
     ...
     */
    private int mismatch(OldNativeSocketAddress other) {
    ...
    public boolean equals(Object other) {
        if (other instanceof NativeSocketAddress) {
            return mismatch((OldNativeSocketAddress) other) < 0;
        } else {
            return false;
        }
    }
